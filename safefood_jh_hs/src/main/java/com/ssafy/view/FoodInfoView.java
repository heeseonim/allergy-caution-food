...
public class FoodInfoView {
	...
	public void setMain() {
		...
		addBt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int code = 0;
				int amount = Integer.parseInt(numberTf.getText());
				int row = foodTable.getSelectedRow();
				System.out.println("선택된 row :" + row);
				System.out.println("선택된 row의 column 값 :" + foodModel.getValueAt(row, 0));
				try {
					code = Integer.parseInt(((String) foodModel.getValueAt(row, 0)).trim());
					showFoodInfo(code);

				} catch (SafeFoodException ue) {
				}
				foodService.addeat(code, amount);
				
				frame2 = new JFrame("Safe Food -- 섭취 영양 정보 관리");
